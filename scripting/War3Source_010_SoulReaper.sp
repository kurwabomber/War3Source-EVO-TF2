...
new Float:ultiDamageMulti[]={1.0,1.1,1.2,1.3,1.4};
...
public OnW3PlayerAuraStateChanged(client,aura,bool:inAura,level,AuraStack,AuraOwner)
{
	...
	if(aura==AuraID)
	{
		...
		if(AuraStack>0 && inAura)
		{
			...
			{
				new Float:StackBuff=(float(AuraStack) * PresenseAmount[level]);
