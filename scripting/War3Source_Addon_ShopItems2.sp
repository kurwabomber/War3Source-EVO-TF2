...
public OnWar3EventDeath(victim, attacker, deathrace, distance, attacker_hpleft)
{
	if(War3_GetOwnsItem2(victim,ItemID[SCROLL_OF_ESSENCE])&&ValidPlayer(victim))
	{
		...
		CreateTimer(0.1,instaspawn,victim);
	}
}
...
#if GGAMETYPE2 == GGAME_MVM
		ItemID[CASH_REGEN]=War3_CreateShopItem2("My Piggy Bank","mvmcashregen","MVM cash regeneration","Cash regeneration.",2000);
#endif
...
public OnMapStart()
{
	//PrecacheSound(MyPiggyBankSound);
	IsMvM(true);
}
