...
//skill 1
new Float:FanOfKnivesTFChanceArr[]={0.2,0.25,0.3,0.35,0.4};
...
{
	if(IS_PLAYER(victim)&&IS_PLAYER(attacker)&&attacker!=victim)
	{
		//VICTIM IS WARDEN!!!
		if(War3_GetRace(victim)==thisRaceID)
		{
			...
			if(!Hexed(victim,false)&&GetRandomFloat(0.0,1.0)<=chance_mod*FanOfKnivesTFChanceArr[skill_level]&&War3_SkillNotInCooldown(victim,thisRaceID,SKILL_FANOFKNIVES))
			{
