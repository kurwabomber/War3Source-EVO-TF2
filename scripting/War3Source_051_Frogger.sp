...
float ProgressiveCost[]={1.0,0.90,0.80,0.70,0.60};

new MaximumWards[]={4,4,5,5,6};
new PushPower[]={4,5,5,6,6};
...
new Float:JumpDistance[]={500.0,550.0,600.0,650.0,700.0};

new Float:SwimSpeed[]={1.5,1.55,1.6,1.65,1.7};
...
public OnWar3PluginReady()
{
	...
	{
		SetFailState("Could not find Hyper Cyborg race on load.");
	}
}
/*
...
public void OnAbilityCommand(int client, int ability, bool pressed, bool bypass)
{
	...
	if(War3_GetRace(client)==thisRaceID && ability==0 && pressed && IsPlayerAlive(client))
	{
		...
		if(!Silenced(client))
		{
			if(War3_GetWardCount(client)<MaximumWards[skill_level])
			{
					...
					if(War3_CreateWardMod(client, location, 90, 120.0, 0.5, "zap", SKILL_BUGZAP_WARD, PushPower, WARD_TARGET_ENEMYS)>-1)
					{
	...
	if(War3_GetRace(client)==thisRaceID && ability==2 && pressed && IsPlayerAlive(client))
	{
		...
		if(!Silenced(client)&&SkillAvailable(client,thisRaceID,SKILL_JUMP,true))
		{
...
stock ReJump(const any:client,bool:extraforce=false) {
	...
	if ( 1 <= g_iJumps[client] <= g_iJumpMax)
	{
		...
		if(extraforce)
		{
			vVel[2] = JumpDistance[skill_level] * 2.0;
		}
		else
		{
...
public Action:SkinFix(Handle:timer, any:sentry)
{
	...
	if(GetEntityClassname(sentry, classname, sizeof(classname)) && StrEqual(classname, "obj_sentrygun", false))
	{
		...
		{
			new client = GetEntDataEnt2(sentry, FindSendPropInfo("CObjectSentrygun","m_hBuilder"));
...
public OnEntityDestroyed(entity) // removed cause player doesn't build sentrys now
{
	...
	else if (IsValidEntity(entity))
	{
		...
		if(GetEntityClassname(entity, classname, sizeof(classname)))
		{
			...
			{
				new client = GetEntDataEnt2(entity, FindSendPropInfo("CObjectSentrygun","m_hBuilder"));
			...
			{
				new client = GetEntDataEnt2(entity, FindSendPropInfo("CObjectDispenser","m_hBuilder"));
