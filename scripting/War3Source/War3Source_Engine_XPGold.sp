...
public bool:War3Source_Engine_XPGold_InitNatives()
{
	...
	CreateNative("W3GetItemReqXP" ,NW3GetItemReqXP);
...
public NW3GetItemReqXP(Handle:plugin,numParams)
{
	new level=GetNativeCell(1);
	if(level>MAXLEVELXPDEFINED)
		level=MAXLEVELXPDEFINED;
	return XPShortTermREQXP[level];
}
...
public NW3GetMaxGold(Handle:plugin,numParams)
{
	...
	} else if (GetPlayerProp(client, W3PlayerProp::PlayerGold) <= GetConVarInt(MaxGoldCvar)) {
		return GetConVarInt(MaxGoldCvar);
	} else {
		return 100000;
	}
