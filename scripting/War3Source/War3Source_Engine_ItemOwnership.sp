	...
	if (ValidPlayer(client))
		{
			...
			if(payforit==true)
			{
				...
				{
					War3_ChatMessage(client,"{default}You have more items than allowed by server.\n{default}Correct adjustments will be made.");
				}
				if(checkit==true)
				{
					War3_ChatMessage(client,"{default}You already{red} own{default} these items{default}.");
					return false;
				}
				counter = 0;
				...
				if(csmoney==true)
					...
					GoldMoney = GetPlayerProp(client, W3PlayerProp::PlayerGold);
				...
				for(int i;i<=ItemsLoaded;i++)
				{
					if(GetOwnsItem(client,i) && !RestoreItemsFromDeath_playerOwnsItem[client][i])
					{
						...
						SetOwnsItem(client,i,false);
						War3_ChatMessage(client,"{default}Item Discarded: {green}%s{default} (You didn't own this item on death).",String_itemName);
					}
				}
				// Find the Items
				counter = 0;
				for(int i;i<=ItemsLoaded;i++)
				{
					...
					if(SkipBuying == false)
					{
						...
						{
							if(!W3IsItemDisabledForRace(GetRace(client),i))
							{
								if(!GetOwnsItem(client,i))
								{
									...
									if(GoldMoney>=War3_Temp_ItemCost)
									{
										...
										GetItemName(i,String_itemName,64);
										War3_ChatMessage(client,"{default}You bought {green}%s{default}.",String_itemName);
										counter++;
									}
									else
									{
										War3_ChatMessage(client,"{default}You can not afford {green}%s{default}.",String_itemName);
									}
								}
								else
								{
									GetItemName(i,String_itemName,64);
									War3_ChatMessage(client,"{default}You already own it! Skipping: {green}%s{default}.",String_itemName);
									counter++;
								}
							}
							else
							{
								GetItemName(i,String_itemName,64);
								War3_ChatMessage(client,"{default}Cannot buy due to restrictions. Skipping: {green}%s{default}.",String_itemName);
								counter++;
							}
						}
					}
				...
				if(csmoney==true)
					...
					SetPlayerProp(client, W3PlayerProp::PlayerGold, GoldMoney);
				// Tell them the total cost
				// To do: if cost == 0 then tell them they didn't buy anything.
				if(csmoney==true)
					War3_ChatMessage(client,"{default}Total cost ${green}%i {default}.",numTotalCost);
				else
					War3_ChatMessage(client,"{default}Total cost {green}%i {default}gold.",numTotalCost);
			}
			else
			{
			//no cost
			...
			for(new i;i<=ItemsLoaded;i++)
			{
				...
				if(RestoreItemsFromDeath_playerOwnsItem[client][i]==true)
				{
					SetOwnsItem(client,i,true);
					GetItemName(i,String_itemName,64);
					War3_ChatMessage(client,"{default}You receive {green}%s{default}.",String_itemName);
					counter++;
				}
	...
	for(new itemid=1;itemid<=ItemsLoaded;itemid++){
		if(GetOwnsItem(client,itemid)){
			...
			if(W3IsItemDisabledForRace(race,itemid)){
				...
				War3_ChatMessage(client,"%T","{itemname} is restricted on {racename}, item has been removed",client,itemname,racename);
