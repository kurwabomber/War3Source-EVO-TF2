...
ShowMenu3Raceinfo(client)
{
	...
	SetMenuTitle(hMenu,"%T\n ","[War3Source:EVO] Select a race for more info",client);
	...
	for(int i=0;i<racedisplay;i++) //notice this starts at zero!
	{
		...
		for(int x=1;x<=RacesLoaded;x++)
		{
			...
			if(FindRaceDependency(raceid,x)>1)
			{
				char tName[32];
				...
				Format(rdisp,sizeof(rdisp),"%s | %s lvl %i",rdisp,tName,FindRaceDependency(raceid,x));
			}
		}
...
public War3_ShowParticularRaceInfoMenu(client,raceid)
{
	...
	SetMenuTitle(hMenu,"%T\n \n","[War3Source:EVO] Information for race: {racename} (LVL {amount}/{amount})",client,racename,War3_GetLevel(client,raceid),GetRaceMaxLevel(raceid));
...
War3_playersWhoAreThisRaceMenu(client,raceid)
{
	...
	SetMenuTitle(hMenu,"%T\n \n","[War3Source:EVO] People who are race: {racename}",client,racename);
		...
		if(found==0){
			...
			for(new i=0;i<found;i++)
			{
				...
				if(StrEqual("No Race",racename,true))
					strcopy(racename, sizeof(racename), "No Race");
