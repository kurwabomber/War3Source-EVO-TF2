...
ShowMenuShopCategory(client)
{
	...
	new gold = GetPlayerProp(client, W3PlayerProp::PlayerGold);
...
War3_TriedToBuyItem(client,item,bool:reshowmenu=true,tomecount=0) {
	...
	if(item>0&&item<=totalItemsLoaded)
	{
		...
		int cred=GetPlayerProp(client, W3PlayerProp::PlayerGold);
		...
		if(canbuy) {
			if(W3IsItemCSmoney(item)) {
				...
				SetPlayerProp(client, W3PlayerProp::PlayerGold, (cred-cost_num));
			}
