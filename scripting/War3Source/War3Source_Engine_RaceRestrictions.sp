...
public ItemRestrictions_PlayerChangeClassEvent(Handle:event,const String:name[],bool:dontBroadcast)
{
	...
	if(userid>0)
	{
		...
		{
			p_properties.Set(client, classid, W3PlayerProp::CurrentClass);
		...
		if(!W3IsDeveloper(client) || (W3IsDeveloper(client)&&!GetConVarBool(gh_AllowDeveloperPowers)))
		{
			...
			if(GetConVarInt(internal_W3GetVar(hRaceLimitEnabledCvar))>0)
			{
				...
				if(GetRace(client)!=race_selected&&GetRacesOnTeam(race_selected,GetClientTeam(client))>=GetRaceMaxLimitTeam(race_selected,GetClientTeam(client))) //already at limit
				{
					...
					if(No_Message==false)
					{
						War3_ChatMessage(client,"Race limit for your team has been reached, please select a different race. (MAX {amount})",GetRaceMaxLimitTeam(race_selected,GetClientTeam(client)));
					}
...
enum TFClassType
{
	...
	TFClass_Engineer
			...
			//PrintToServer("cvarid %d",cvarid); //holy shit this console spam is unbearable.
