...
public Action:Timer_UpdateInfo(Handle:timer)
{
	if(!MapChanging && !War3SourcePause)
	{
		...
		for(new i;i<=MaxClients;i++)
		{
			...
			if(ValidPlayer(i) && !IsFakeClient(i) && ( GetPlayerProp(i,W3PlayerProp::iGoldDiamondHud)==1))
			{
				//PrintToServer("%i",i);
				...
				gold=GetPlayerProp(i, W3PlayerProp::PlayerGold);
				...
				if(GetPlayerProp(i,W3PlayerProp::iRotateHUD)==1||GetPlayerProp(i,W3PlayerProp::iRotateHUD)==2) {
					if(RotateTimer[i]<=0.0) {
						RotateTimer[i]=GetEngineTime();
					}
					if(RotateTimer[i]<GetEngineTime()) {
						if (GetPlayerProp(i,W3PlayerProp::iRotateHUD)==2)
							RotateNum[i]++;
						else
				...
				else if(GetPlayerProp(i,W3PlayerProp::iRotateHUD)==0)
				{
				...
				else if(GetPlayerProp(i,W3PlayerProp::iRotateHUD)==3)
				{
...
public Action:Timer_Diamonds(Handle:timer, any:userid)
{
	if(!MapChanging && !War3SourcePause)
	{
		for(int i=1; i<MaxClients; i++)
		{
			...
			{
				int GivePlayerDiamonds = War3_GetDiamonds(i) + GetConVarInt(DiamondsPerMinute);
