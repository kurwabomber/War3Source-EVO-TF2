...
War3Source_UpdateStats()
{
	if(hDB)
	{
		...
		iTopCount=0;
		SQL_TQuery(hDB,T_RetrieveTopCallback,"SELECT steamid,name,total_level,total_xp FROM war3source_evo ORDER BY total_xp DESC,total_xp DESC LIMIT 0,100");
	}
	else
	{
		PrintToServer("wtf... lame ass update stats doesnt work...");
	}
}
...
GetRank(client)
{
	...
	else
	{
		...
		PrintToChat(client,"Getting your rank...");
		SQL_TQuery(hDB,T_RetrieveRankCache,"SELECT steamid FROM war3source_evo ORDER BY total_xp DESC,total_xp DESC",GetClientUserId(client));
	}
}
...
public T_RetrieveRankCache(Handle:owner,Handle:query,const String:error[],any:userid)
{
	...
	{
		PrintToServer("invalid query... | Error: %s", error);
	}
}
...
public War3Source_Stats_Player(client,target)
{
	if(ValidPlayer(target,false))
	{
		...
		new goldtitle=GetPlayerProp(target, W3PlayerProp::PlayerGold);
		...
		SetMenuTitle(playerInfo,"[War3Source:EVO] Info for %s. Current race: %s gold: %d",playername,race_name,goldtitle);
...
War3Source_War3Top(client,top_num,cur_place=0)
{
	...
	for(new x=cur_place;x<99;x++)
	{
		...
		if(x2<10)
		{
			...
			Format(menuText,sizeof(menuText),"%s%d - %s (Lvl. %d, %d XP)\n",menuText,x+1,Top100Name[x],Top100totallevel[x],Top100totalxp[x]);
			x2++;
		}
/*		if(ValidPlayer(x))
		{
