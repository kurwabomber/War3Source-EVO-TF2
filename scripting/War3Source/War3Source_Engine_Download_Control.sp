...
UpdateDownloadControl()
{
	...
	else
	{
		...
		// HIGH
		new iCount=0;
		new iMaxDownloadsNow=iMaxDownloads-RoundToCeil((float(iMaxDownloads) * GetConVarFloat(ihighdownloadsCvar)));
		if(GetConVarBool(EnableHighDownloadsCvar))
		{
			for(new i = 0; i < GetArraySize(g_hSoundFile); i++)
			{
		...
		iMaxDownloadsNow=RoundToCeil((float(iMaxDownloadsNow) * GetConVarFloat(imediumdownloadsCvar)));
		iCount=0;
		if(GetConVarBool(EnableMediumDownloadsCvar))
		{
			for(new i = 0; i < GetArraySize(g_hSoundFile); i++)
		...
		// PRIORITY_LOW
		iMaxDownloadsNow=iMaxDownloads-(iMaxDownloadsNow-iCount);
		iMaxDownloadsNow=RoundToCeil((float(iMaxDownloadsNow) * GetConVarFloat(ilowdownloadsCvar)));
		iCount=0;
		if(GetConVarBool(EnableLowDownloadsCvar))
		{
			for(new i = 0; i < GetArraySize(g_hSoundFile); i++)
		...
		// PRIORITY_BOTTOM
		iMaxDownloadsNow=iMaxDownloads-(iMaxDownloadsNow-iCount);
		iMaxDownloadsNow=RoundToCeil((float(iMaxDownloadsNow) * GetConVarFloat(ibottomdownloadsCvar)));
		iCount=0;
		if(GetConVarBool(EnableBottomDownloadsCvar))
		{
			for(new i = 0; i < GetArraySize(g_hSoundFile); i++)
