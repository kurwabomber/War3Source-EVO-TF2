...
public void War3Source_Engine_WCX_Engine_Crit_OnWar3EventPostHurt(int victim, int attacker,float damage,char weapon[64],bool isWarcraft,int weaponID)
{
	...
	float DamageMultiplier = GetBuffSumFloat(attacker, fDamageModifier) + ((IsValidEntity(weaponID) && TF2Util_IsEntityWeapon(weaponID) && TF2Util_GetWeaponSlot(weaponID) != TFWeaponSlot_Melee) ? GetBuffSumFloat(attacker, fDamageModifierRanged) : 0.0);
