...
W3BuffProperties BuffProperties[W3Buff];
...
stock internal_SetBuffAny(int client,W3Buff buffindex,int itemraceindex,any value,int buffowner=-1)
{
	...
	if(buffindex==fMaxSpeed||buffindex==fSlow||buffindex==fMaxSpeed2||buffindex==fSlow2)
	{
		...
		if(ValidPlayer(client) && GetPlayerProp(client,W3PlayerProp::iBuffChatInfo2)==1)
		{
...
ShowInvisBuff(client)
{
	...
	{
		new Float:currentAttribute=(Float:BuffCached[client][fInvisibilitySkill] * Float:BuffCached[client][fInvisibilityItem]);
		if(currentAttribute>1.0)
			currentAttribute=(currentAttribute * 1.0);
		else
			currentAttribute=(1.0 * currentAttribute);
...
ShowArmorPhysical(client)
{
	if(ValidPlayer(client))
	{
		...
		new percentage=RoundToFloor((PhysicalArmorMulti(client) * 100.0));
...
ShowArmorMagic(client)
{
	if(ValidPlayer(client))
	{
		...
		new percentage=RoundToFloor((MagicArmorMulti(client) * 100.0));
		percentage=100-percentage;
		if(currentAttribute>0.0)
...
ShowVampireBuff(client)
{
	if(ValidPlayer(client))
	{
		...
		if(currentAttribute>1.0)
			...
			currentAttribute=(1.0 - currentAttribute);
		currentAttribute=currentAttribute*100.0;
		new percentage=RoundToFloor((100.0 - currentAttribute));
		if(currentAttribute>0.0)
...
ShowSpeedBuff(client,bool:bypass=false)
{
	if(ValidPlayer(client))
	{
		...
		new Float:NEWcurrentmaxspeed=(currentmaxspeed/TF2_GetClassSpeed(TF2_GetPlayerClass(client)));
		if(NEWcurrentmaxspeed>1.0)
			NEWcurrentmaxspeed=(NEWcurrentmaxspeed - 1.0);
		else
			NEWcurrentmaxspeed=(1.0 - NEWcurrentmaxspeed);
...
{
	if(ValidPlayer(client) && !IsFakeClient(client) && GetPlayerProp(client,W3PlayerProp::iBuffChatInfo)==1)
	{
