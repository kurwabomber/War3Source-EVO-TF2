...
{
	hUseCategories = CreateConVar("war3_jobcats","1","If non-zero job categories will be enabled");
	hAllowCategoryDefault = CreateConVar("war3_allow_default_cats","0","Allow Default categories to show in category menu? (default 0)");
	hAllowAllRacesCategory = CreateConVar("war3_allow_all_races_cats","1","Allow Default categories to show in category menu? (default 1)");
...
War3Source_ChangeRaceMenu(client,bool:forceUncategorized=false)
{
	if(W3IsPlayerXPLoaded(client))
	{
		...
		if( IsCategorized() && !forceUncategorized ) {
			...
			Format(title,sizeof(title),"%s%T",title,"[War3Source:EVO] Select your desired race",GetTrans()) ;
			if(W3GetLevelBank(client)>0){
				Format(title,sizeof(title),"%s\n%T\n",title,"You Have {amount} levels in levelbank. Say levelbank to use it",GetTrans(), W3GetLevelBank(client));
			}
			SetMenuTitle(crMenu,"%s\n \n",title);
			...
			char rname[128];
			char rdisp[128], requirement[128];
			...
			for(int i=0;i<racedisplay;i++) //notice this starts at zero!
			{
				...
				Format(rname,sizeof(rname),"%s",rname);
				...
				else
				{
					...
					Format(rdisp,sizeof(rdisp),"%s",rdisp);
				}
#else
	...
	switch(action) {
		case MenuAction_Select:
		{
			if(ValidPlayer(client))
			{
				...
				SetMenuExitButton(crMenu,true);
				Format(title,sizeof(title),"%T","[War3Source:EVO] Select your desired race",GetTrans());
				SetMenuTitle(crMenu,"%s\nCategory: %s\n",title,sItem);
				// Iteriate through the races and print them out
				int racelist[MAXRACES];
				int racedisplay=GetRaceList(racelist);
		...
		case MenuAction_Select:
		{
			...
			/* MOVED TO RESTRICT ENGINE
				...
				*/
				...
				if(allowChooseRace)
				{
					SetPlayerProp(client,W3PlayerProp::RaceChosenTime,GetGameTime());
					SetPlayerProp(client,W3PlayerProp::RaceSetByAdmin,false);

					//has race, set pending,
					if(GetRace(client)>0&&IsPlayerAlive(client))
					{
						...
						if(!JailBreak)
						{
							...
							if(gh_CVAR_AllowInstantSpawn.BoolValue && TF2Util_IsPointInRespawnRoom(position))
							{
								W3SetPendingRace(client,-1);
								SetRace(client,race_selected);
								War3_ChatMessage(client,"You can now changerace instantly in spawn.");
							}
							else
							{
								War3_ChatMessage(client,"You must be in {green}spawn{default} to have instant race changing.",buf);
