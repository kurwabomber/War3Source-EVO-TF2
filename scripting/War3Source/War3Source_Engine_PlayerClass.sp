...
public War3Source_Engine_PlayerClass_OnPluginStart()
{
	...
	p_properties = new ArrayList(sizeof(W3PlayerProp), MaxClients+1);
}
...
stock SetRace(client,newrace)
{
	...
	{
		LogError("WARNING SET INVALID RACE for client %d to race %d",client,newrace);
		return;
	}
	if (client > 0 && client <= MaxClients)
	{
		new oldrace=p_properties.Get(client, W3PlayerProp::CurrentRace);
		if(oldrace==newrace){
			//WTF ABORT
			return;
		}
			...
			internal_W3SetVar(OldRace,p_properties.Get(client, W3PlayerProp::CurrentRace));

			if(oldrace>0&&ValidPlayer(client)){
				W3SaveXP(client,oldrace);
			}

			p_properties.Set(client,newrace, W3PlayerProp::CurrentRace);
...
{
	if (client > 0 && client <= MaxClients)
		return p_properties.Get(client, W3PlayerProp::CurrentRace);
...
stock GetPlayerProp(client,any property)
{
	if (client > 0 && client <= MaxClients)
	{
		return p_properties.Get(client, property);
	}
	else
		return 0;
}

public any NW3GetPlayerProp(Handle:plugin,numParams)
{
	return GetPlayerProp(GetNativeCell(1),any:GetNativeCell(2));
}

stock SetPlayerProp(client,any property,any:value)
{
	if (client > 0 && client <= MaxClients)
	{
		p_properties.Set(client, value, property);
	}
}

public NW3SetPlayerProp(Handle:plugin,numParams){
	SetPlayerProp(GetNativeCell(1),any:GetNativeCell(2),any:GetNativeCell(3));
}
...
public Event_PlayerTeam(Handle:event,  const String:name[], bool:dontBroadcast)
{
	...
	SetPlayerProp(client,W3PlayerProp::LastChangeTeamTime,GetEngineTime());
}
...
public Action:cmdwar3notdev(client,args){
	if(ValidPlayer(client)){
		SetPlayerProp(client,W3PlayerProp::isDeveloper,false);
...
public War3Source_Engine_PlayerClass_OnWar3Event(W3EVENT:event,client)
{
	if(event==InitPlayerVariables)
	{
		...
		if(GetConVarBool(gh_AllowDeveloperAccess))
		{
			...
			{
				SetPlayerProp(client,W3PlayerProp::isDeveloper,true);    // Default is true
			}
		}
		...
		SetPlayerProp(client,W3PlayerProp::PendingRace,0);
		SetRace(client,0); //need the race change event fired
		SetPlayerProp(client,W3PlayerProp::dbRaceSelected,false);
		SetPlayerProp(client,W3PlayerProp::PlayerGold,0);
		War3_SetDiamonds(client,0);
		War3_SetPlatinum(client,0);
		SetPlayerProp(client,W3PlayerProp::iMaxHP,0);
		SetPlayerProp(client,W3PlayerProp::bIsDucking,false);

		SetPlayerProp(client,W3PlayerProp::RaceChosenTime,0.0);
		SetPlayerProp(client,W3PlayerProp::RaceSetByAdmin,false);
		SetPlayerProp(client,W3PlayerProp::SpawnedOnce,false);
		SetPlayerProp(client,W3PlayerProp::sqlStartLoadXPTime,0.0);
		SetPlayerProp(client,W3PlayerProp::isDeveloper,false);
		SetPlayerProp(client,W3PlayerProp::LastChangeTeamTime,0.0);
		SetPlayerProp(client,W3PlayerProp::bStatefulSpawn,true);
