...
ShowMenu3ShopCategory(client)
{
	...
	Format(title,sizeof(title),"%s\n Current Race: %s\n",title, racename);
	Format(title,sizeof(title),"%s\n Gems are locked to whatever race you buy them with.\n",title);
...
ShowMenu3Shop3(client, const String:category[]=""){
	...
	SetMenuExitBackButton(shopMenu3,true);
	new Platinum=War3_GetPlatinum(client);
...
InternalGemSlotItemsMenuBuy(client)
{
	...
	if(display)
		...
		SetMenuTitle(hInternalMenu,"You can only have one [%s] item per race.",categorySTR);
...
InternalExceededMaxItemsMenu3Buy(client)
{
	...
	if(itemsexists)
		...
		SetMenuTitle(hMenu,"You can only have one [%s] item per race.",categorySTR);
...
War3_playertargetItemMenu3(client,target){
		...
		if(War3_GetOwnsItem3(target,race,itemid1))
		{
			...
			else
			{
				...
				itemMaxXp=W3GetItemReqXP(itemlvl+1);
