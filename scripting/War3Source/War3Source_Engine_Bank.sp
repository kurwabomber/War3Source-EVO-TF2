...
enum struct W3Bank
{
	bool hasbank;
	int W3Bank_gold;
	int W3Bank_timestamp;
}

//new String:p_bank_steamid[MAXPLAYERSCUSTOM][64];

W3Bank p_bank[MAXPLAYERSCUSTOM];
...
public NWar3_DepositGoldBank(Handle:plugin,numParams){
	...
	if (client > 0 && client <= MaxClients)
	{
		...
		int currentgold=GetPlayerProp(client, W3PlayerProp::PlayerGold);
...
public NWar3_WithdrawGoldBank(Handle:plugin,numParams){
	...
	if (client > 0 && client <= MaxClients)
	{
		...
		{
			int playerGold=GetPlayerProp(client, W3PlayerProp::PlayerGold);
...
stock char[] withdrawalTime1(time)
{
	int withdrawnumber=GetConVarInt(WithDrawTimeLimitCvar);
