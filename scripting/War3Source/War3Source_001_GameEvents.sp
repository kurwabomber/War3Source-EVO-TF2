...
public War3Source_PlayerSpawnEvent(Handle:event,const String:name[],bool:dontBroadcast)
{
	...
	if(userid>0)
	{
		...
		if(ValidPlayer(client,true))
		{
			...
			if(!GetPlayerProp(client,W3PlayerProp::SpawnedOnce))
			{
				SetPlayerProp(client,W3PlayerProp::SpawnedOnce,true);
			}
			...
			SetPlayerProp(client,W3PlayerProp::bStatefulSpawn,false); //no longer a "stateful" spawn
		}
	}
	return 0;
}
...
public Action:War3Source_PlayerDeathEvent(Handle:event,const String:name[],bool:dontBroadcast)
{
	...
	if(victim>0&&attacker>0)
	{
		...
		distance = RoundToNearest((calcDistance(victimLoc[0],attackerLoc[0], victimLoc[1],attackerLoc[1], victimLoc[2],attackerLoc[2]) * 12.0));
	...
	//	DP("new event %d",event);
		...
		SetPlayerProp(victimIndex,W3PlayerProp::bStatefulSpawn,true);//next spawn shall be stateful
