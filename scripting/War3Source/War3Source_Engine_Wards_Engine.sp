// War3Source_Engine_Wards_Engine.sp
...
new Handle:g_OnWardNotTriggerHandle = INVALID_HANDLE;
new Handle:g_OnWardExpireHandle = INVALID_HANDLE;

new g_iPlayerWardCount[MAXPLAYERSCUSTOM];

public War3Source_Engine_Wards_Engine_OnPluginStart()
{
	HookEvent("round_end", War3Source_Engine_Wards_Engine_Event_RoundEnd);
}

public War3Source_Engine_Wards_Engine_OnClientConnected(client)
{
	g_iPlayerWardCount[client] = 0;
}

public bool:War3Source_Engine_Wards_Engine_InitNativesForwards()
{
	g_OnWardCreatedHandle = CreateGlobalForward("OnWardCreated", ET_Ignore, Param_Cell, Param_Cell);
	g_OnWardPulseHandle = CreateGlobalForward("OnWardPulse", ET_Ignore, Param_Cell, Param_Cell, Param_Cell);
	g_OnWardTriggerHandle = CreateGlobalForward("OnWardTrigger", ET_Ignore, Param_Cell, Param_Cell, Param_Cell, Param_Cell);
	g_OnWardNotTriggerHandle = CreateGlobalForward("OnWardNotTrigger", ET_Ignore, Param_Cell, Param_Cell, Param_Cell, Param_Cell);
	g_OnWardExpireHandle = CreateGlobalForward("OnWardExpire", ET_Ignore, Param_Cell, Param_Cell, Param_Cell);

	return true;
}

public bool:War3Source_Engine_Wards_Engine_InitNatives()
{
	...
	CreateNative("War3_IsWardDistanceTooClose", Native_War3_IsWardDistanceTooClose);
...
{
	new iRadius=GetArrayCell(g_hWardRadius,GetNativeCell(1));
#if SHOPMENU3 == MODE_ENABLED
	new owner=GetArrayCell(g_hWardOwner,GetNativeCell(1));
	if(ValidPlayer(owner))
	{
		new itemitemlevel=War3_GetItemLevel(owner, GetRace(owner), War3_GetItem3IdByShortname("brilamber"));
		iRadius+=(itemitemlevel*10);
	}
#endif
	return iRadius;
}
...
public Native_War3_CreateWardMod(Handle:plugin, numParams)
{
	...
	if(W3Denyable(DN_CanPlaceWard, client)||bypass)
	{
		new id = PushArrayCell(g_hWardOwner, client);
...
public WardPulse(id)
{
	...
	for(new i=1; i <= MaxClients; i++)
	{
		if(ValidPlayer(i, true))
		{
			...
			{
				// Took this compare out to solve issue with very tall wards:
				// now compare z
			...
			{
				Call_StartForward(g_OnWardNotTriggerHandle);
				Call_PushCell(id);
				Call_PushCell(i);
				Call_PushCell(owner);
				Call_PushCell(GetArrayCell(g_hWardBehavior, id));
...
public War3Source_Engine_Wards_Engine_OnClientDisconnect(client)
{
	RemoveWards(client);
}

public War3Source_Engine_Wards_Engine_OnWar3EventSpawn(client)
{
	//RemoveWards(client);
	bool bDisableWard;
	for(new i=0; i < GetArraySize(g_hWardOwner); i++)
	{
		if (GetArrayCell(g_hWardOwner, i) == client)
		{
			bDisableWard = GetArrayCell(g_hWardDisableOnDeath, i);
			if (bDisableWard)
			{
...
public War3Source_Engine_Wards_Engine_OnRaceChanged(client,oldrace,newrace)
{
	RemoveWards(client);
}

public War3Source_Engine_Wards_Engine_Event_RoundEnd(Handle:event, const String:name[], bool:dontBroadcast)
{
	for(new i=0; i < GetArraySize(g_hWardOwner); i++)
	{
		RemoveWard(i);
...
public War3Source_Engine_Wards_Engine_OnGameFrame()
{
	...
	//if(GetArraySize(g_hWardOwner)<=0)
	//{
		//return;
	//}

	//if(GetArraySize(g_hWardEnabled)<=0)
	//{
		//return;
	//}

	// may need to put i to 1 if errors
	for(new i = 0; i < GetArraySize(g_hWardOwner); i++)
	{
...
//native bool:War3_IsWardDistanceTooClose(client,DistanceCheck);
public Native_War3_IsWardDistanceTooClose(Handle:plugin, numParams)
{
	new Float:location[3];
	new bool:bEnabled;
	//GetNativeArray(1,location,3);
	new client=GetNativeCell(1);
	GetClientAbsOrigin(client,location);
	new Float:MaxDistance=Float:GetNativeCell(2);
	...
	for(new i = 0; i < GetArraySize(g_hWardOwner); i++)
	{
		...
		GetArrayArray(g_hWardLocation, i, location2);
		if (GetVectorDistance(location,location2) < MaxDistance)
		{
			return true;
		}
	}
